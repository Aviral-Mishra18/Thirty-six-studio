import "./index.css";
import Canvas from "./Canvas";
import data from "./data";
import LocomotiveScroll from "locomotive-scroll";
import { useEffect, useState, useRef } from "react";
import { useGSAP } from "@gsap/react";
import gsap from "gsap";

function App() {
  const [showCanvas, setShowCanvas] = useState(false);
  const headingref = useRef(null);
  const growingSpan = useRef(null);
  const textContainerRef = useRef(null);

  useEffect(() => {
    const locomotiveScroll = new LocomotiveScroll();
  }, []);

  useGSAP(() => {
    headingref.current.addEventListener("click", (e) => {
      setShowCanvas(!showCanvas);
      if (!showCanvas) {
        gsap.set(growingSpan.current, {
          top: e.clientY,
          left: e.clientX,
        });

        gsap.to("body", {
          color: "#000",
          backgroundColor: "#a04e4cff",
          duration: 1.2,
          ease: "power2.inOut",
        });

        gsap.to(textContainerRef.current, {
          color: "#000",
          duration: 1.2,
          ease: "power2.inOut",
        });

        gsap.to(growingSpan.current, {
          scale: 1000,
          backgroundColor: "#a04e4cff",
          duration: 2,
          ease: "power2.inOut",
        });
      } else {
        gsap.to("body", {
          color: "#fff",
          backgroundColor: "#000",
          duration: 1.2,
          ease: "power2.inOut",
        });

        gsap.to(growingSpan.current, {
          scale: 0,
          backgroundColor: "#000",
          duration: 2,
          ease: "power2.inOut",
        });

        gsap.to(textContainerRef.current, {
          color: "#fff",
          duration: 1.2,
          ease: "power2.inOut",
        });
      }
    });
  }, [showCanvas]);

  return (
    <>
      <span
        ref={growingSpan}
        className="growing rounded-full block fixed top-[-20px] left-[-20px] w-5 h-5">
      </span>
      <div className="w-full relative min-h-screen font-['Helvetica_Now_Display']">
        {showCanvas &&
          data[0].map((canvasdets, index) => (
            <Canvas key={index} details={canvasdets} />
          ))}
        <div ref={textContainerRef} className="w-full relative  z-[1] h-screen text-white">
          <nav className="w-full p-8 flex justify-between z-50">
            <div className="brand text-2xl font-regular">thirtysixstudios</div>

            <div className="links flex gap-10">
              {["Home", "About", "Projects", "Contact"].map((link, index) => (
                <a
                  key={index}
                  href={`#${link.toLowerCase()}`}
                  className="text-base hover:text-gray-300"
                >
                  {link}
                </a>
              ))}
            </div>
          </nav>
          <div className="textcontainer w-full px-[20%]">
            <div className="text w-[50%]">
              <h3 className="text-4xl leading-[1.2]">
                At Thirtysixstudio, we build digital assets and immersive experiences for brands purpose.
              </h3>
              <p className="text-lg w-[80%] mt-10 font-normal">
                We're a boutique production studio focused on design, animation, and technology, constantly rethinking what digital craft can do for present-day ads and campaigns.</p>
              <p className="text-md mt-10">scroll</p>
            </div>
          </div>

          <div className="w-full absolute bottom-0 left-0">
            <h1
              ref={headingref}
              className="text-[15rem] font-normal tracking-tighter leading-none pl-5 select-none cursor-pointer">
              Thirtysixstudios
            </h1>
          </div>
        </div>
      </div>
      <div className="w-full relative h-screen z-[10] mt-32 px-10">
        <h1 className="text-8xl  tracking-tighter">about the brand</h1>
        <p className="text-4xl  leading-[1.8] w-[80%] mt-10 font-light">
          We are a team of designers, developers, and strategists who are passionate about creating digital experiences that are both beautiful and functional.
        </p>
        {data[1].map((canvasdets, index) => (
          <Canvas key={index} details={canvasdets} />
        ))}

        <img
          className="w-[80%] mt-10"
          src="https://directus.funkhaus.io/assets/b3b5697d-95a0-4af5-ba59-b1d423411b1c?withoutEnlargement=true&fit=outside&width=1400&height=1400" alt="" />
      </div>
    </>
  );
}

export default App;